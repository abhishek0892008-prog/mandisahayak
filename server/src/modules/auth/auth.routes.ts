...
import { badRequest, ErrorCodes, forbidden, unauthenticated } from '../../core/errors.ts';
...
function parse<T extends z.ZodTypeAny>(schema: T, body: unknown): z.infer<T> {
  ...
  if (!result.success) {
    throw badRequest(ErrorCodes.VALIDATION_FAILED, 'Request validation failed', zodFields(result.error));
  }
  return result.data;
}

/** Audits a rate-limit rejection before surfacing it. */
async function rejectRateLimited(req: Request, outcome: NonNullable<Awaited<ReturnType<typeof consumeAll>>>) {
...
export function buildAuthRouter(): Router {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const limited = await consumeAll([
        { rule: RateLimits.REGISTER_PER_IP, subject: req.clientIp ?? 'unknown' },
        { rule: RateLimits.OTP_SEND_PER_PHONE, subject: input.phone },
        { rule: RateLimits.OTP_SEND_PER_PHONE_DAILY, subject: input.phone },
        { rule: RateLimits.OTP_SEND_PER_IP, subject: req.clientIp ?? 'unknown' },
      ]);
      if (limited) throw await rejectRateLimited(req, limited);

      const view = await withTransaction((client) => startRegistration(client, input, ctxOf(req)));
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const limited = await consumeAll([
        { rule: RateLimits.STAFF_LOGIN_PER_USERNAME, subject: input.username },
        { rule: RateLimits.STAFF_LOGIN_PER_IP, subject: req.clientIp ?? 'unknown' },
      ]);
      if (limited) throw await rejectRateLimited(req, limited);

      const view = await withTransaction((client) =>
        startStaffLogin(client, input.username, input.password, ctxOf(req)),
      );
      sendData(res, 201, view);
    }),
  );
  ...
  declareRoute({
    ...
    auth: { kind: 'public', reason: 'Completes authentication; a session does not exist yet.' },
    csrf: true,
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const limited = await consumeAll([
        { rule: RateLimits.OTP_VERIFY_PER_IP, subject: req.clientIp ?? 'unknown' },
      ]);
      if (limited) throw await rejectRateLimited(req, limited);

      // The service manages its own transactions: failed-attempt bookkeeping
      // must commit even though the request fails.
      const result = await verifyOtpAndCreateSession(input.challengeId, input.otp, ctxOf(req));
  ...
  declareRoute({
    ...
    auth: { kind: 'public', reason: 'Operates on an unauthenticated challenge.' },
    csrf: true,
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      await withTransaction((client) =>
        logoutService(client, actor.sessionId, actor.userId, actor.roles, ctxOf(req)),
      );
  ...
  declareRoute({
    method: 'GET',
    path: `${BASE}/auth/csrf`,
    auth: { kind: 'public', reason: 'Issues the CSRF token needed before any authenticated write.' },
    csrf: false,
  ...
  router.get('/auth/csrf', (req, res) => {
    ...
    const current = issued ?? (req as unknown as { cookies: Record<string, string> }).cookies?.fq_csrf;
  ...
  declareRoute({
    ...
    auth: { kind: 'public', reason: 'DEMO_MODE only; guarded by a static dev token and audited.' },
    csrf: false,
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      const cfg = getConfig();
      if (!cfg.DEMO_MODE) throw forbidden(ErrorCodes.FORBIDDEN, 'Not available');
      ...
      if (!parsed.success) {
        throw badRequest(ErrorCodes.VALIDATION_FAILED, 'phone required', zodFields(parsed.error));
      }
