...
import { formatDateTime, formatQuantity, kgToQuintal } from "../../lib/format";
...
/**
 * Procurement record for one booking.
 *
 * Every value here is written by an officer at the centre and read back
 * verbatim. Nothing is derived in the browser — not the accepted quantity, not
 * the grade, not whether the crop passed.
 *
 * A `404` is the expected answer before an officer records arrival, so it is
 * rendered as "not started yet" rather than as a failure.
 */
function Procurement() {
  ...
  const notStarted = record.error instanceof ApiError && record.error.status === 404;
  const procurement = record.data?.procurement ?? null;
  const payment = record.data?.payment ?? null;

  const quintal = (kg) => (kg === null || kg === undefined ? null : kgToQuintal(kg));
    ...
    >
        ...
        <section className="mb-4 rounded-2xl bg-white p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h2 className="truncate font-semibold text-slate-900">{booking.centre?.name}</h2>
      ...
      {notStarted && (
        <EmptyState
          ...
          description={t("procurementNotStartedDescription")}
        />
      )}
        ...
        <>
          <section className="rounded-2xl bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <h2 className="font-semibold text-slate-900">{t("procurement")}</h2>

              <StatusBadge
                status={procurement.status}
                label={translateProcurementStatus(t, procurement.status)}
              />
            </div>
            ...
            <div className="mt-2 divide-y divide-slate-100">
              <DetailRow
                ...
                value={translateQualityStatus(t, procurement.qualityStatus)}
              />
              ...
              <DetailRow
                ...
                value={
                  procurement.grossQuantityKg === null
                    ? null
                    : `${formatQuantity(quintal(procurement.grossQuantityKg), locale)} ${t("quintal")}`
                }
              />
              ...
              <DetailRow
                ...
                value={
                  procurement.acceptedQuantityKg === null
                    ? null
                    : `${formatQuantity(quintal(procurement.acceptedQuantityKg), locale)} ${t("quintal")}`
                }
              />
              ...
              <DetailRow
                ...
                value={
                  procurement.rejectedQuantityKg === null
                    ? null
                    : `${formatQuantity(quintal(procurement.rejectedQuantityKg), locale)} ${t("quintal")}`
                }
              />

              <DetailRow label={t("grade")} value={procurement.grade} />
              ...
              <DetailRow
                ...
                value={
                  procurement.moisturePercent === null
                    ? null
                    : `${formatQuantity(procurement.moisturePercent, locale)}%`
                }
              />

              {procurement.rejectionReason && (
                <DetailRow label={t("rejectionReason")} value={procurement.rejectionReason} />
              )}
            </div>
          </section>

          <section className="mt-4 rounded-2xl bg-white p-4 shadow-sm">
            <h2 className="font-semibold text-slate-900">{t("statusLabel")}</h2>

            <div className="mt-2 divide-y divide-slate-100">
              <DetailRow
                label={t("arrivedAt")}
                value={formatDateTime(procurement.arrivedAt, booking?.centre?.timezone, locale)}
              />
              ...
              <DetailRow
                label={t("completedAt")}
                value={formatDateTime(procurement.completedAt, booking?.centre?.timezone, locale)}
              />
            </div>
          </section>
          ...
          {payment && (
            <button
              type="button"
              onClick={() => navigate("/payment", { state: { bookingCode } })}
              className="mt-4 flex w-full items-center justify-between rounded-2xl border border-green-100 bg-green-50 px-4 py-4 text-left"
            >
              <div>
                <p className="text-xs font-semibold text-green-700">{t("payment")}</p>
